...
% Author: Luca Vezzaro (elvezzaro@gmail.com)
	...
	while i < numel(varargin)
		if strcmp(varargin{i}, 'triangulation')
			...
			shape_triangles = double(varargin{i+1});
		else
    ...
    %--------------------------EXERCISE AFTER ALIGNMENT  -----------
    
    
    figure(1), imshow(app_data(:,:,:,1));
	%%
	%% DEBUG STUFF
	%
   ...
   %--------------------------EXERCISE AFTER ALIGNMENT  -----------
    ...
    % Exircise 2.2 : Training shapes after the alignment using Procrustes Analysis.
    figure(2), imshow(app_data(:,:,:,1));
    hold on;
	for i=1:size(aligned_data, 3)
